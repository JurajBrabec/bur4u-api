...
const SEPARATOR = ',';
const QUOTE = '"';
const ESL_EXIT_CODE = 4;

const LINE1 = '#Automatic upload for backup information to ESL';
const LINE2 = `#Generated by BUR4U-API v${version} @ `;
const FULL_BACKUP = 0;
...
const primary = ({ masterServer, policies }) =>
  policies
    .reduce(
      ...
      [LINE1, LINE2 + timeStamp(), Object.keys(PRIMARY_FIELDS).join(SEPARATOR)]
    )
  ...
  policies
    .reduce(
      ...
      [
        LINE1,
        LINE2 + timeStamp(),
        Object.keys(SECONDARY_FIELDS).join(SEPARATOR),
      ]
    )
    .join(DELIMITER) + DELIMITER;
